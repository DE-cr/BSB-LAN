...
# if so, remove the '#' at the beginning of the following line(s):
...
temperature_columns = find_parameter_columns(df, "°")
if temperature_columns:
    df.ffill().plot(x="Date", y=temperature_columns, grid=True, figsize=(19.2, 10.8))
    plt.tight_layout()
    plt.show()
...
# Note: You may have to adjust the parameter numbers used here to those
#       in your logged data! (Look for 'find_parameter_columns' below!)

# locate some parameter columns we're using:
columns = find_parameter_columns(df, 8700)
outside_temperature_column = columns[0] if columns else False

# let's see if we find some flow temperature parameter in the log:
columns = find_parameter_columns(df, 8744)
if not columns:
    columns = find_parameter_columns(df, 8743)
if not columns:
    columns = find_parameter_columns(df, 8774)
if not columns:
    columns = find_parameter_columns(df, 8773)
flow_temperature_column = columns[0] if columns else False


# plotting rolling averages of outside temperatures:
if outside_temperature_column:
...
df["date"] = df.Date.dt.date  # just the date, without time
...
if outside_temperature_column and flow_temperature_column:
    for year in df.year.unique():
        dfx = df.query(f"year=={year}")
        plt.scatter(
            dfx[outside_temperature_column],
            dfx[flow_temperature_column],
            0.1,
            label=year,
        )
    plt.xlabel(outside_temperature_column)
    plt.ylabel(flow_temperature_column)
...
if flow_temperature_column:
    calendar_plot(df, "time", flow_temperature_column)


# bar plot of burner activity hours by year/month:
columns = find_parameter_columns(df, 8005)
burner_status_column = columns[0] if columns else False
samples_per_hour = 60  # recommend (adjust, if necessary)

if burner_status_column:
